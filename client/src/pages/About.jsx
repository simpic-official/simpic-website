        ...
        <section className='flex flex-col mt-[10em]'>
          ...
          <div className='flex flex-col items-center mt-[3em] gap-5'>
            {['2024', '2023', '2022', '2021'].map(year => (
              <div 
                key={year}
                className={`transition-all duration-300 ease-in-out w-full max-w-[516px] hover:-translate-y-1 hover:scale-105 ${open[`simpic${year}`] ? "max-h-[129px]" : "max-h-[84px]"} flex gap-3 items-center px-4 py-4 bg-gradient-to-r from-[rgba(251,166,166,1)] to-[rgba(232,201,122,1)] rounded-lg cursor-pointer`} 
                onClick={() => changeDisplay(`simpic${year}`)}
              >
                <p className='flex items-center text-[24px] font-bold'>{`SIMPIC${year}`}</p>
                <p className={`${open[`simpic${year}`] ? "w-[1px] h-[80px] bg-black" : "hidden"}`}></p>
                <p className={`${open[`simpic${year}`] ? "block text-xs lg:text-md" : "hidden"}`}>
